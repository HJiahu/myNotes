...
*	要提前准备好提问时要问的问题（网上搜集公司与职位的资料，公司成立时间、主要业务、职位要求等），最好要有明确的职位需要。不要问与自己职位相差太远的问题，例如公司未来5年的战略。不要在技术面的时候问薪水，这个问题应该问HR。因为面试有好几轮，只有所有的都过了才可能被聘。最好问与职位相关的问题。设身处地的想一想，如果你成为了这个团队的一员，你会遇到哪些问题，你想知道什么？问题最好与职位或项目相关。
...
*	一些数据结构
	...
	*	堆：堆（也叫优先队列），在内存中堆用一个数组保存，但我们可以将其看做是一棵完全二叉树，它的特点是父节点的值大于（小于）两个子节点的值（分别称为大顶堆和小顶堆）。为了便于计算，堆的起始index是1而非0，这样通过（无符号型）左右移位可以快速计算出左右节点（还有父节点）的index（使用宏或inline函数可以加快这个操作）
		*	在堆中，数组的长度并不代表合法的堆中元素的个数，最少数组下标0所代表的元素不属于堆。
...
#### 问题汇总
*	在C++中对含有虚析构函数的空对象使用sizeof其结果是多少
...
*	面试题5 逆序打印单向链表
	...
	> 两种典型的思想：使用栈做缓冲；使用递归的思想。一般使用非递归的方式实现递归操作都会涉及到栈。
