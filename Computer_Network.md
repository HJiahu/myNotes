...
-	每一台路由器都有一个  ***转发表***
-	最长前缀匹配规则：
> 有4亿台主机。那么当一台主机向另一台发送消息时，若每一个路由器都维护一个有4亿条记录的转发表，这个量太大了，很难实现。              
...
***IP数据报格式***
-	版本号（路由器确定解析方式）。
-	首部长度。大多数的数据报中没有可选项（则首部长度为20字节），但有些数据报中有可选项，故需要这个数据来确定这些数据报的首部长度。
-	服务类型。区别不同类型的IP数据报。
-	数据报长度。
-	标识，标志，片偏移。
-	寿命（TTL time to live），确定数据报不会永远在网络中循环。
-	协议。指明IP数据报应该交给哪个运输层协议。（6标识TCP   ，  17标识UDP）
-	首部校验和。
-	源和目的IP地址。
-	选项。允许IP数据报被扩展的字段（IPv6中不再使用）。
-	数据。有效载荷。
...
-	分片的原理：片的组装是在端完成的，路由器只负责分片，而不组装片。
	...
	-	数据报在从源主机发出的时候，每一个IP都有一个标号：标识。对于同一个数据，源主机每发送一个IP标识都会加一。
...
-	a.b.c.d/x  ，x 表示a.b.c.d的前x位为网络前缀。剩余32-x位是用来区分组织内部设备的。
