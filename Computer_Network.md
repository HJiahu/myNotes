...
网络的分层结构：
-	应用层：
	-	应用层协议：DNS（53）、HTTP（80） 、SMTP（25）、FTP（20  21 ） ... ***应用层协议主要关注的是数据的内容，和数据的传输没有关系。***
	-	应用层数据：报文（message）
	-	应用层性能：可靠数据传输（TCP）、吞吐量、定时、安全性（SSL：  secure socket layer）
-	传输层：
	-	传输层协议：TCP（面向链接）、UDP（面向报文）   ***从这一层就开始讨论数据的传输问题。***
	-	传输层数据：报文段（segment）
-	网络层：
	-	网络层协议：IP
	-	网络层数据：数据报（datagram）
...
***UDP套接字和TCP套接字的一些区别***
> TCP是面向连接的，故其套接字中有四元数据组用来保存目的地IP目的地端口号，源ip源端口号，这些数据用来确定数据的来源。            
> UDP是面向报文的，它只是尽最大可能传输数据，故在UDP报文中只使用一个两元的数据组用来区别数据的来源。      
> 从应用的角度，对于tcp而言，只有四元组中的所有数据都匹配，这个报文才会定向到对应的套接字，以确定数据是来自一个管道。然而UDP不是这样，只要目的端口和目的ip匹配，数据就会被重定向对应的套接字，无论数据的来源ip是否相同。
...
运输层的工作方式：
> 运输层扫描获得的运输层报文，从中获取相关信息，不同的运输层报文重新定向到不同的进程的套接字，这个过程称为多路分解，而反过程称为多路复用。一般在运输层报文段中有目的端口号和源端口号。
