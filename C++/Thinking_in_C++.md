...
- tips
	-	头文件中不应该使用using指令。
	-	声明使得名字被程序所知，而定义则是创建与名字相关的实体。`extern int` 为声明而 `int j` 和 `extern int k = 0;`为定义。在函数体内试图初始化一个由关键字extern约束的变量是非法的，因为函数内部的局部变量是无法被外部访问的，故不能在函数内部定义全局变量（而初始化一个由extern约束的变量就是在定义全局变脸），但可以声明全局变量。
