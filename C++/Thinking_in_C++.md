...
- tips
	...
	-	endl 被称为操纵符，写入endl的效果是介绍当前行并且***将与设备关联的缓冲区中的内容刷入到设备中。这可以保证所有需要显示的数据已经写入输入流而没有保存在内存中。***
	-	字面值，只能使用其值来称呼它。
	...
	-	auto：***audo 定义的变量必须有初始值，否则编译器无法进行推导。***若auto i = 0;那么编译器会推导i为整型。***auto在推导的过程中会忽略对象的顶层const，但会留下底层const特性。***
